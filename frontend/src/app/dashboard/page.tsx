...
export default function DashboardCaseSelectionPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchCases = async () => {
      ...
      // Admins can see all cases, officers see only assigned cases
      const url =
        user.role === "administrator"
          ? `${API_URL}/cases`
          : `${API_URL}/cases?assignedTo=${user.id}`;
        ...
        <div className="flex flex-col items-center justify-center h-64 bg-muted/30 rounded-lg">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            {user?.role === "officer"
              ...
              : "Create a new case from the admin panel"}
          </p>
        </div>
      ) : (
